 ...
 *
 * `processDocument` runs the pipeline on a real uploaded file.
 * `processSampleDocument` runs the SAME pipeline on a bundled demo
 * transcription: only the OCR stage is substituted (no OCR call is faked),
 * every later stage is identical and the audit trail records that the text
 * came from a labelled demo sample.
 */
import { createServerFn } from "@tanstack/react-start";
import { z } from "zod";

import { extractFields, extractionConfidence } from "./pipeline/extraction";
import { findSample } from "./pipeline/samples";
...
type OcrOutcome = { text: string; confidence: number; engine: string; documentKind: string };

/**
 * Stages 2-6, shared by real uploads and demo samples. Nothing here knows how
...
async function finalize(args: {
  ...
  const { ocr, filename, storagePath, intakeDetail } = args;
  ...
  const audit = await supabaseAdmin.from("audit_events").insert([
    {
      record_id: recordId,
      event_type: "upload",
      ...
      detail: intakeDetail,
      actor: "System",
      after_value: storagePath ?? filename,
      created_at: at(0),
    },
    {
      record_id: recordId,
  ...
  if (audit.error) {
    throw new Error("The record was saved but its audit trail could not be written.");
  }
...
export const processDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ProcessResult> => {
    ...
    return finalize({
      ...
      filename,
      storagePath,
      intakeDetail: `"${filename}" (${(bytes.byteLength / 1024).toFixed(0)} KB, ${ocr.documentKind.toUpperCase()}) was validated and stored securely as ${storagePath}.`,
    });
  });

/**
 * Demo samples: the transcription is bundled, so stage 1 is a lookup rather
 * than an OCR call. The record is clearly labelled as a demo sample and the
 * text is stored in the sample bucket so officers can reopen it.
 */
export const processSampleDocument = createServerFn({ method: "POST" })
  .inputValidator((data: unknown) => z.object({ sampleId: z.string().min(1) }).parse(data))
  .handler(async ({ data }): Promise<ProcessResult> => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const sample = findSample(data.sampleId);
    if (!sample) throw new Error("That demo sample is not available.");

    const bytes = new TextEncoder().encode(sample.text);
    const storagePath = `samples/${Date.now()}-${sample.filename}`;
    ...
    return finalize({
      ocr: {
        ...
        engine: "bundled demo transcription (no OCR call)",
        documentKind: "demo sample",
      },
      filename: sample.filename,
      storagePath,
      intakeDetail: `Demo sample "${sample.filename}" was loaded from the bundled scenario library and stored as ${storagePath}. The transcription is pre-supplied, so no OCR call was made; every later stage ran normally.`,
    });
  });
